...
import { normalizePDFSource, validateBase64PDF, extractPDFMetadata, detectPDFSourceType } from '../utils/pdf-utils';
...
export class PDFEngine {
    ...
    try {
      ...
      if (sourceType === 'base64') {
        ...
        try {
          ...
          console.log(`Loading PDF from base64: ${Math.round(metadata.size / 1024)}KB, version: ${metadata.version || 'unknown'}`);
        } catch (metaError) {
      ...
      console.log(`PDF loaded successfully: ${this.pdfDocument.numPages} pages, fingerprint: ${this.pdfDocument.fingerprints?.[0] || 'unknown'}`);
    } catch (error) {
      ...
      if (error instanceof Error) {
        if (error.message.includes('Invalid PDF')) {
          ...
          throw new Error('Failed to load PDF from URL - check network connection and file availability');
        }
      }
  ...
  async renderPage(
    ...
    if (pageData.rendered && pageData.canvas && pageData.scale === scale) {
      console.log(`PDF Engine: Page ${pageNumber} already rendered at scale ${scale}, returning cached`);
      return pageData.canvas;
    }
    
    if (pageData.rendered && pageData.scale !== scale) {
      console.log(`PDF Engine: Page ${pageNumber} scale changed from ${pageData.scale} to ${scale}, re-rendering`);
