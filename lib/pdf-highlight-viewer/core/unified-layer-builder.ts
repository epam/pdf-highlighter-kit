...
export class UnifiedLayerBuilder {
  ...
  private findIntersectingTextItems(
    textItems: TextItem[],
    coordinates: BoundingBox
  ): Array<{ item: TextItem; startIndex: number; endIndex: number; itemIndex: number }> {
    const intersecting: Array<{ item: TextItem; startIndex: number; endIndex: number; itemIndex: number }> = [];
    let globalIndex = 0;
  ...
  private getHighlightsForTextItem(
    ...
    highlights: HighlightData,
    pageNumber: number
  ): Array<{ termId: string; category: string; coordinates: BoundingBox }> {
    const itemHighlights: Array<{ termId: string; category: string; coordinates: BoundingBox }> = [];
  ...
  private createHighlightedSegments(
    textItem: TextItem,
    itemHighlights: Array<{ termId: string; category: string; coordinates: BoundingBox }>
  ): Segment[] {
    const segments: Segment[] = [];
    const itemBounds = this.getTextItemBounds(textItem);
