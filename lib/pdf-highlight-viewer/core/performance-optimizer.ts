...
export class RTree implements SpatialIndex {
  ...
  insert(bounds: BoundingBox, data: any): void {
    ...
    let node = this._chooseSubtree(bounds, this.root, insertPath);
  ...
  private _findItem(bounds: BoundingBox, data: any, node: RTreeNode, path: RTreeNode[]): RTreeItem | null {
    path.push(node);
    
    if (node.leaf) {
      return (node.children as RTreeItem[]).find(item => 
        item.data === data && this._boundsEqual(item.bounds, bounds)
      ) || null;
    }
  ...
  private estimateSize(data: any): number {
    ...
    if (typeof data === 'object') {
      return Object.keys(data).reduce((sum, key) => 
        sum + this.estimateSize(key) + this.estimateSize(data[key]), 0
      );
    }
    return 1000;
  }
