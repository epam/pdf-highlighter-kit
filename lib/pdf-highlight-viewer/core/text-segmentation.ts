...
export class TextSegmentation {
  ...
  segmentText(
    ...
    const segmentationPoints = this.findSegmentationPoints(
      textContent.items,
      highlightRanges
    );
  ...
  private mapHighlightsToCharacters(
    ...
    Object.entries(highlights).forEach(([category, categoryData]) => {
      ...
      pageHighlights.forEach((occurrence, occIndex) => {
        occurrence.coordinates.forEach((coord, coordIndex) => {
  ...
  private resolveOverlappingHighlights(ranges: HighlightRange[]): HighlightRange[] {
    ...
    for (const range of ranges) {
      const overlapping = resolved.filter(existing => 
        this.rangesOverlap(range, existing)
      );
  ...
  private calculateHighlightPriority(category: string, occurrenceIndex: number): number {
    const categoryPriorities: { [key: string]: number } = {
      'protein': 100,
      'gene': 90,
      'disease': 80,
      'chemical': 70,
  ...
  private createSegment(
    ...
    const overlappingHighlight = highlightRanges.find(range =>
